...
async function getCounts() {
  try {
    const res = await fetch(`${API_URL}/stats/counts`, { cache: 'no-store' });
    if (!res.ok) return { players: 0, clubs: 0, games: 0, competitions: 0 };
    return res.json();
  } catch { return { players: 0, clubs: 0, games: 0, competitions: 0 }; }
}

async function getRecentMatches() {
  try {
    const res = await fetch(`${API_URL}/games?limit=4`, { cache: 'no-store' });
    if (!res.ok) return [];
    return res.json();
  } catch { return []; }
}
...
export default async function Home() {
  const [counts, recentMatches] = await Promise.all([getCounts(), getRecentMatches()]);
      ...
      <section className="relative w-full py-24 lg:py-40 overflow-hidden flex items-center justify-center min-h-[70vh]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center">
          ...
          <div className="animate-fade-in-up glass rounded-full px-5 py-2 text-sm font-medium text-emerald-300 mb-8 inline-flex items-center gap-2">
            ...
            Base de datos con +{formatNumber(counts.games)} partidos históricos
          </div>
          ...
          <p className="animate-fade-in-up delay-300 text-lg md:text-xl text-emerald-100/80 max-w-2xl font-light mb-12 leading-relaxed">
            Explora resultados históricos, descubre el rendimiento de tus equipos favoritos y analiza las carreras de los mejores jugadores del mundo.
          </p>
          ...
          {/* Stats reales */}
          <div className="animate-fade-in-up delay-700 mt-16 grid grid-cols-4 gap-6 max-w-2xl mx-auto">
            <div className="glass rounded-2xl p-4 text-center hover-lift cursor-default">
              <p className="text-2xl md:text-3xl font-black text-emerald-400">{formatNumber(counts.games)}</p>
              <p className="text-xs text-emerald-200/60 mt-1 uppercase tracking-wider">Partidos</p>
            </div>
            <div className="glass rounded-2xl p-4 text-center hover-lift cursor-default">
              <p className="text-2xl md:text-3xl font-black text-teal-400">{formatNumber(counts.clubs)}</p>
              <p className="text-xs text-emerald-200/60 mt-1 uppercase tracking-wider">Equipos</p>
            </div>
            <div className="glass rounded-2xl p-4 text-center hover-lift cursor-default">
              <p className="text-2xl md:text-3xl font-black text-emerald-300">{formatNumber(counts.players)}</p>
              <p className="text-xs text-emerald-200/60 mt-1 uppercase tracking-wider">Jugadores</p>
            </div>
            <div className="glass rounded-2xl p-4 text-center hover-lift cursor-default">
              <p className="text-2xl md:text-3xl font-black text-teal-300">{counts.competitions}</p>
              <p className="text-xs text-emerald-200/60 mt-1 uppercase tracking-wider">Competiciones</p>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full">
        <div className="animate-fade-in-up flex justify-between items-end mb-10">
          <div>
            ...
            <p className="text-zinc-500 dark:text-zinc-400 mt-2">Los últimos encuentros registrados en la base de datos</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {recentMatches.length > 0 ? (
            ...
            <p className="text-zinc-500 col-span-4 text-center">Cargando partidos...</p>
          )}
        </div>
      </section>
    </div>
